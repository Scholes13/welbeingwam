-- Fix photo bonus points calculation in check_in_spot function
...
BEGIN
  ...
  -- Use spot's base points, fallback to settings
  v_base_points := v_spot.points;
  
  -- Get photo bonus from settings (stored as JSON with 'points' key)
  SELECT (value->>'points')::INTEGER INTO v_photo_bonus 
  FROM settings 
  WHERE key = 'photo_bonus_points';
  
  v_photo_bonus := COALESCE(v_photo_bonus, 25);
